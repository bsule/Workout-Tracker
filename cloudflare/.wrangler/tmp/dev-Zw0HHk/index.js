...
async function derive(password, salt, iterations, bytes) {
...
async function createToken(db, userId) {
  ...
  await db.prepare("INSERT INTO tokens (token, user_id) VALUES (?, ?)").bind(token, userId).run();
  return token;
}
__name(createToken, "createToken");
async function lookupToken(db, token) {
  const row = await db.prepare(
    "SELECT u.id AS id, u.username AS username, u.email AS email FROM tokens t JOIN users u ON u.id = t.user_id WHERE t.token = ?"
  ).bind(token).first();
  return row ?? null;
}
__name(lookupToken, "lookupToken");
...
var requireAuth = /* @__PURE__ */ __name(async (c, next) => {
  ...
  const match2 = header.match(/^Token\s+(.+)$/i);
  if (!match2) return c.json({ detail: "Authentication credentials were not provided." }, 401);
...
auth.patch("/me/", requireAuth, async (c) => {
  ...
  const body = await readJson(c.req.raw);
  if (!body) return c.json({ detail: "Invalid JSON" }, 400);
  const nextUsername = body.username !== void 0 ? str(body.username).trim() : null;
  const nextEmail = body.email !== void 0 ? str(body.email).trim() : null;
  if (nextUsername !== null) {
    if (!nextUsername) return c.json({ username: ["This field is required."] }, 400);
    const conflict = await c.env.DB.prepare("SELECT id FROM users WHERE username = ? COLLATE NOCASE AND id != ?").bind(nextUsername, me.id).first();
    if (conflict) {
      return c.json({ username: ["A user with that username already exists."] }, 400);
    }
  }
  if (nextEmail !== null) {
    ...
    const conflict = await c.env.DB.prepare("SELECT id FROM users WHERE email = ? COLLATE NOCASE AND id != ?").bind(nextEmail, me.id).first();
    if (conflict) {
      return c.json({ email: ["A user with that email already exists."] }, 400);
    }
  }
  ...
  const updated = await c.env.DB.prepare("SELECT id, username, email FROM users WHERE id = ?").bind(me.id).first();
...
var objectKey = /* @__PURE__ */ __name((userId) => `users/${userId}/snapshot.bin`, "objectKey");
...
sync.put("/snapshot", async (c) => {
  ...
  if (!ifMatch && ifNoneMatch !== "*") {
    return c.json(
      {
        detail: "If-Match: <etag> (overwrite) or If-None-Match: * (first write) required."
      },
      428
    );
  }
...
// .wrangler/tmp/bundle-3dnMA4/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-3dnMA4/middleware-loader.entry.ts
