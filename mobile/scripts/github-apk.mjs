#!/usr/bin/env node
// Triggers the "Build Android (APK)" GitHub Actions workflow, waits for it to
// finish, and downloads the resulting .apk into mobile/builds/. The APK is
// debug-signed and installs directly on any Android device.
...
async function main() {
  ...
  for (const f of readdirSync(outDir)) {
    if (f.endsWith(".apk")) unlinkSync(resolve(outDir, f));
  }
  console.log("downloading artifact...");
  gh(["run", "download", String(runId), "--name", ARTIFACT, "--dir", outDir], { stdio: "inherit" });

  const apk = readdirSync(outDir).filter((f) => f.endsWith(".apk")).map((f) => resolve(outDir, f));
  console.log("done. files:");
  apk.forEach((p) => console.log("  " + p));
}
