/**
 * Local CSV/JSON exporters that read from the IndexedDB-backed snapshot.
 * No network calls — everything runs in the browser.
 */

import type { Snapshot } from "@/lib/store/schema"
import { SEED_EXERCISES } from "@/lib/store/seed"

/** Mirror of the backend exporter columns (one row per set). */
const CSV_HEADERS = [
  "date",
  "exercise",
  "category",
...
function csvCell(v: unknown): string {
  ...
  if (typeof v === "number") {
    ...
    // Strip trailing zeros / decimal point for nicer CSV.
...
export function buildCsv(snap: Snapshot): string {
  ...
  const sets = snap.sets
    ...
    .filter((row): row is { s: typeof row.s; we: NonNullable<typeof row.we>; w: NonNullable<typeof row.w> } =>
...
function timestampedName(format: "csv" | "json" | "fitnotesdb"): string {
  const t = new Date()
    .toISOString()
    .replace(/[:.]/g, "")
    .replace("T", "-")
    .slice(0, 15)
  if (format === "fitnotesdb") return `FitNotes-${t}.fitnotesdb`
  return `lift-export-${t}.${format}`
}

export function downloadCsv(snap: Snapshot) {
  downloadBlob(buildCsv(snap), timestampedName("csv"), "text/csv;charset=utf-8")
}

export function downloadJson(snap: Snapshot, username = "") {
  downloadBlob(
    buildJson(snap, username),
    timestampedName("json"),
    "application/json"
  )
}
