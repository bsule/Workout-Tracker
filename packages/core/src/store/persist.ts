...
export function setStorageFactory(f: StorageFactory) {
  storageFactory = f
}
...
function ensureStorage(): BlobStorage {
  if (!storage) {
    if (!storageFactory) {
      throw new Error(
        "lift/core: storage factory not set. Call setStorageFactory(...) before hydrate()."
      )
    }
...
export function recordPending(op: unknown) {
  if (!storageFactory) return // not configured (e.g. SSR)
...
/**
 * Host apps (web/mobile) call this from their lifecycle hooks (visibilitychange,
 * pagehide, AppState) to force a synchronous flush before the process is
 * suspended. Cancels any pending debounced flush.
 */
