...
import {
  recomputePrsForExercise,
  recomputePrsForExercises,
  recomputePrsForWe,
} from "./prs"
...
export function deleteWorkout(id: number): void {
  applyMutation((snap) => {
    ...
    // Every exercise that had sets on this day may have lost a record.
    return recomputePrsForExercises(
      next,
...
export function removeExerciseFromWorkout(
  ...
  applyMutation((snap) => {
    // Resolve the exercise before the WE is filtered out — afterwards there
    // is no row left to walk from.
    const we = snap.workout_exercises.find((x) => x.id === weId)
    if (!we) return snap
    const next = {
      ...snap,
      workout_exercises: snap.workout_exercises.filter((x) => x.id !== weId),
      sets: snap.sets.filter((s) => s.workout_exercise_id !== weId),
    }
    // Dropping the WE drops its sets, so a record it held has to move on to
    // the runner-up — exactly as deleteSet does.
    return recomputePrsForExercise(next, we.exercise_id)
  })
...
export function copyFromWorkout(
  ...
  applyMutation((snap) => {
    ...
    if (newSets.length === 0) return next
    // Copied sets land with is_pr false. Usually they're dominated by the
    // originals they were copied from, but not always: copying onto an
    // *earlier* date makes the copy win the exact-tie rule (earliest wins),
    // so the flags have to be recomputed rather than assumed.
    return recomputePrsForExercises(
      next,
      newWes.map((we) => we.exercise_id)
    )
  })
...
// ---- PRs ----------------------------------------------------------
// The per-exercise passes live in ./prs so persist.ts can replay them.
