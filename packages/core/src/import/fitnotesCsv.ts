/**
 * FitNotes Android CSV importer (local-first). Pure logic — accepts CSV text
 * (not a File object) so it can run in both web and React Native hosts.
 *
...
export async function importFitnotesCsv(
  ...
  if (rows.length === 0) {
    if (mode === "replace") {
      // Even an empty CSV is a valid replace target — wipe and persist.
  ...
  // appending sets to a workout-exercise that already has sets. Skipped in
  // replace mode since the surviving sets list will be empty.
  if (mode === "merge") {
