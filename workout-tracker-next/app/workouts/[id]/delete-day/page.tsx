...
import { getWorkoutById } from "@/lib/mock-data"
import { formatDate } from "@/lib/utils"

export default function DeleteDayPage() {
  const params = useParams<{ id: string }>()
  const workout = getWorkoutById(Number(params.id))
  const [selected, setSelected] = useState<string>("")

  const selectedSession = workout?.sessions.find(
    (s) => String(s.id) === selected
  )

  function handleDelete() {
    // Backend not connected yet
    console.log("delete session", selected)
  }
